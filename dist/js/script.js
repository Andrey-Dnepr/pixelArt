    ...
    let clickCount = [], //флаг для отслеживания - будет ли ли кисть рисовать в режиме long step или нет
        ...
        setPixel = function(e) {
            if (e.target.classList.contains('pixel')) {
                !erase ? e.target.classList.add('pixelCircle') : e.target.classList.remove('pixelCircle'); //проверяет режим, если режим стирания - до очищает пиксель, иначе закрашивает
                clickCount[0] = 1;
            }
        },
        ...
        if (event.target.classList.contains('pixel')) {
            if (!stepChoice) {
                if (clickCount[0]) {
                    ...
                    !erase ? event.target.classList.add('pixelCircle') : event.target.classList.remove('pixelCircle'); //проверяет режим, если режим стирания - то очищает пиксель, иначе закрашивает
                ...
                !erase ? event.target.classList.add('pixelCircle') : event.target.classList.remove('pixelCircle'); //проверяет режим, если режим стирания - до очищает пиксель, иначе закрашивает
            }
        }
    });
